...
IMPORT = "import '../features/downloads/downloads_panel.dart';\n"
RETIRED = ["_DownloadsCard", "_DownloadRow"]
...
def main() -> None:
    ...
    if IMPORT not in text:
        ...
        if anchor not in text:
            raise RuntimeError("Downloads presentation import anchor missing")
        text = text.replace(anchor, anchor + IMPORT, 1)

    old = "          _DownloadsCard("
    new = "          WzDownloadsPanel("
    if text.count(old) != 1:
        raise RuntimeError("Expected exactly one active _DownloadsCard callsite")
    ...
    for name in RETIRED:
        if re.search(rf"\b{re.escape(name)}\b", text):
            raise RuntimeError(f"Refusing wiring: retired Downloads symbol still referenced: {name}")
    if text.count("WzDownloadsPanel(") != 1:
        raise RuntimeError("Expected exactly one WzDownloadsPanel callsite")
    if text == original:
        raise RuntimeError("Wiring produced no change")

    TARGET.write_text(text, encoding="utf-8")
    print(f"Wired Downloads panel; net line delta: {text.count(chr(10)) - original.count(chr(10))}.")
